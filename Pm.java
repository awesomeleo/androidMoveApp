...
public final class Pm {
    ...
    class PackageDeleteObserver extends IPackageDeleteObserver.Stub {
        ...
        //2.2的接口
        //line 55 @ http://grepcode.com/file/repository.grepcode.com/java/ext/com.google.android/android/2.2_r1.1/android/content/pm/IPackageDeleteObserver.java 
        public void packageDeleted(boolean isSuccessful) {
            synchronized (this) {
                finished = true;
                result = isSuccessful;
                notifyAll();
    ...
    class PackageMoveObserver extends IPackageMoveObserver.Stub {
		...
		public void packageMoved(String packageName, int returnCode)
			...
			synchronized (this) {
				finished = true;
				result = returnCode == PackageManager.MOVE_SUCCEEDED;
				notifyAll();
			}
		}
    ...
    private void runMove() {
    	...
    	while((opt=nextOption()) != null) {
    		if(opt.equals("-l")) {
    			moveFlags = PackageManager.MOVE_INTERNAL;
    		}
    	}
    	...
    	boolean result = movePackage(pkg, moveFlags);
        if (result) {
            System.out.println("Success");
        } else {
            System.out.println("Failure");
    ...
    private boolean movePackage(String pkg, int moveFlags) {
    	PackageMoveObserver obs = new PackageMoveObserver();
    	try {
    		mPm.movePackage(pkg, obs, moveFlags);
    		synchronized (obs) {
    			while(!obs.finished) {
    				try {
    					obs.wait();
    ...
    private static void showUsage() {
        ...
        System.err.println("");
        System.err.println("pm move: moves a package between internal and external storage. Options:");
        System.err.println("    -l: move from external to internal storage.");
        System.err.println("");
